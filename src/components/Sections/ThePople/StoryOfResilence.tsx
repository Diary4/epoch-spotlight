  ...
  return (
    <main ref={rootRef} className="m-0 min-h-[100dvh] w-full max-w-[100vw] overflow-x-hidden bg-[#fbf3e8] text-[#174b3d]">
      <section className="relative mx-auto flex min-h-0 w-full max-w-[1400px] flex-col overflow-x-hidden overflow-y-auto rounded-[22px] bg-[#fff7ec] sm:rounded-[28px] lg:min-h-[calc(100vh-2*clamp(10px,1.6vh,24px))] lg:overflow-hidden lg:rounded-[clamp(22px,2.4vw,34px)]">
        ...
        <div className="hero-image pointer-events-none absolute inset-x-0 top-[000px] h-[60vh] sm:top-[clamp(100px,10vh,180px)] sm:h-[min(58vh,520px)] lg:right-0 lg:left-auto lg:top-[clamp(130px,11vh,210px)] lg:h-[clamp(780px,67vh,1160px)] w-full">
          <img
            src={bg1}
        ...
        <section
          dir={isRtl ? "rtl" : "ltr"}
          className="relative z-10 w-full min-w-0 max-w-none px-3 pb-4 pt-12 sm:px-6 sm:py-6 lg:mt-[clamp(62px,8.2vh,126px)] lg:max-w-[min(58vw,760px)] lg:px-[clamp(20px,5vw,80px)] lg:py-[clamp(20px,5vh,80px)]"
        >
        ...
        <section className="relative z-20 mt-[150px] sm:mt-[200px] grid w-full grid-cols-3 items-stretch gap-1.5 xs:gap-2.5 sm:gap-5 pb-4 pt-4 px-3 xs:px-6 sm:px-6 lg:mt-[clamp(20px,4.2vh,70px)] lg:gap-[clamp(16px,1.8vw,34px)] lg:px-14 lg:pb-[clamp(8px,1vh,22px)] lg:pt-[clamp(20px,2.8vh,42px)]">
          {localizedCards.map((card) => {
            const Icon = card.icon;
            return (
              <article
                key={card.title}
                className="resilience-card relative flex min-h-[140px] xs:min-h-[180px] sm:min-h-[300px] lg:min-h-[clamp(330px,30vh,500px)] flex-col items-center overflow-hidden rounded-[12px] border border-[#f4e5cc] bg-white/82 px-1.5 py-4 xs:px-2.5 xs:py-5 sm:border-2 sm:rounded-[24px] sm:px-6 sm:py-7 lg:rounded-[clamp(20px,2.2vw,32px)] lg:px-[clamp(16px,1.8vw,34px)] lg:py-[clamp(18px,2.1vh,34px)] text-center shadow-[0_4px_12px_rgba(84,54,16,0.1)] sm:shadow-[0_14px_35px_rgba(84,54,16,0.16)] backdrop-blur-md"
              >
