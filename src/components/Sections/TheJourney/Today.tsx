    ...
    <main ref={rootRef} className="m-0 min-h-screen w-full max-w-none bg-[#f8f1e7] text-[#17233b] overflow-x-hidden">
      <section className="relative mx-auto flex min-h-screen w-full max-w-[1400px] flex-col overflow-hidden sm:rounded-[22px] bg-[#fbf5eb]">
        ...
        <button
          type="button"
          onClick={onBack}
          className="journey-detail-back absolute left-4 top-4 sm:left-[clamp(1rem,2vw,2rem)] sm:top-[clamp(1rem,2vh,2rem)] z-30 grid h-10 w-10 sm:h-[clamp(2.8rem,4.4vw,3.8rem)] sm:w-[clamp(2.8rem,4.4vw,3.8rem)] place-items-center rounded-full border border-[#d9b477] sm:border-2 bg-white/70 text-[#17233b] shadow-sm transition-transform active:scale-95"
          aria-label="Back to Journey"
        >
          <ArrowLeft className="h-5 w-5 sm:h-7 sm:w-7 lg:h-8 lg:w-8" />
        </button>
        
        {/* Decorative Grid Overlays (Hidden on narrow screens) */}
        <div className="absolute left-0 top-[120px] h-full w-12 sm:w-24 opacity-10 sm:opacity-25 [background-image:linear-gradient(45deg,#d6b56e_1px,transparent_1px),linear-gradient(-45deg,#d6b56e_1px,transparent_1px)] [background-size:22px_22px] hidden sm:block" />
        <div className="absolute right-0 top-[120px] h-full w-12 sm:w-24 opacity-10 sm:opacity-20 [background-image:linear-gradient(45deg,#d6b56e_1px,transparent_1px),linear-gradient(-45deg,#d6b56e_1px,transparent_1px)] [background-size:22px_22px] hidden sm:block" />

        {/* Hero Background Image */}
        <div className="pointer-events-none absolute right-0 top-0 h-[45vh] sm:h-[min(100vh,1500px)] w-full z-0">
          <img
            src={bg}
              alt="Today development"
              className="journey-detail-hero absolute inset-0 h-full w-full object-cover [mask-image:linear-gradient(to_bottom,black_70%,transparent_100%)] sm:[mask-image:radial-gradient(circle_at_60%_48%,black_0%,black_55%,transparent_84%)]"
          />
        </div>
