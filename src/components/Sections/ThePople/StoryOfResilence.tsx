...
export default function StoryOfResilience({ lang = "en", onBack }: StoryOfResilienceProps) {
  ...
  return (
    <main ref={rootRef} className="m-0 min-h-[100dvh] w-full max-w-[100vw] overflow-x-hidden bg-[#fbf3e8] text-[#174b3d]">
      <section className="relative mx-auto flex min-h-0 w-full max-w-[1400px] flex-col overflow-x-hidden overflow-y-auto rounded-[22px] bg-[#fff7ec] sm:rounded-[28px] lg:min-h-[calc(100vh-2*clamp(10px,1.6vh,24px))] lg:overflow-hidden lg:rounded-[clamp(22px,2.4vw,34px)]">
        <button
          type="button"
          onClick={onBack}
          className="back-btn absolute left-4 top-4 z-30 grid h-12 w-12 place-items-center rounded-full border-2 border-[#d9b477] bg-white/70 text-[#174b3d] shadow-sm sm:left-6 sm:top-6 sm:h-14 sm:w-14 lg:left-[clamp(16px,2vw,30px)] lg:top-[clamp(16px,2vh,30px)] lg:h-[clamp(50px,4.8vw,64px)] lg:w-[clamp(50px,4.8vw,64px)]"
        ...
        <div className="hero-image pointer-events-none absolute inset-x-0 top-24 z-0 h-[min(48vh,420px)] w-full sm:top-[clamp(100px,10vh,180px)] sm:h-[min(58vh,520px)] lg:right-0 lg:left-auto lg:top-[clamp(130px,11vh,210px)] lg:h-[clamp(780px,67vh,1160px)]">
          <img
            src={bg1}
        ...
        <section
          dir={isRtl ? "rtl" : "ltr"}
          className="relative z-10 w-full min-w-0 max-w-none px-4 pb-4 pt-20 sm:px-6 sm:py-6 lg:mt-[clamp(62px,8.2vh,126px)] lg:max-w-[min(58vw,760px)] lg:px-[clamp(20px,5vw,80px)] lg:py-[clamp(20px,5vh,80px)]"
        >
          <h1 className="main-title font-serif text-[clamp(40px,11vw,116px)] font-light leading-[1.05] tracking-tight text-[#214439] sm:leading-[1.03]">
          ...
          <div className="title-divider mt-6 flex items-center gap-3 text-[#c9903f] sm:mt-[clamp(26px,3.8vh,48px)] sm:gap-[clamp(14px,1.5vw,22px)]">
            <span className="h-0.5 w-20 bg-[#c9903f] sm:w-[clamp(130px,13vw,220px)]" />
            <span className="text-xl sm:text-[clamp(24px,2.2vw,36px)]">✥</span>
            <span className="h-0.5 w-16 bg-[#c9903f] sm:w-[clamp(110px,11vw,190px)]" />
          </div>

          <p className="subtitle-text mt-5 font-serif text-[clamp(22px,5.5vw,52px)] leading-tight text-[#b06f25] sm:mt-[clamp(22px,3vh,40px)]">
          ...
          <p className="description-text mt-5 max-w-none text-[clamp(15px,4vw,30px)] font-light leading-[1.65] text-[#35435b] sm:mt-[clamp(18px,2.8vh,38px)] sm:leading-[1.75] lg:max-w-[min(48vw,650px)]">
            {detail?.description ??
        ...
        <section className="relative z-20 mt-6 grid w-full grid-cols-1 items-stretch gap-4 px-4 pb-8 pt-2 sm:mt-10 sm:gap-5 sm:px-6 sm:pb-10 sm:pt-4 md:grid-cols-3 lg:mt-[clamp(20px,4.2vh,70px)] lg:gap-[clamp(16px,1.8vw,34px)] lg:px-14 lg:pb-[clamp(8px,1vh,22px)] lg:pt-[clamp(20px,2.8vh,42px)]">
          {localizedCards.map((card) => {
            const Icon = card.icon;
            return (
              <article
                key={card.title}
                className="resilience-card relative flex min-h-[260px] flex-col items-center overflow-hidden rounded-[20px] border-2 border-[#f4e5cc] bg-white/82 px-5 py-6 text-center shadow-[0_14px_35px_rgba(84,54,16,0.16)] backdrop-blur-md sm:min-h-[300px] sm:rounded-[24px] sm:px-6 sm:py-7 lg:min-h-[clamp(330px,30vh,500px)] lg:rounded-[clamp(20px,2.2vw,32px)] lg:px-[clamp(16px,1.8vw,34px)] lg:py-[clamp(18px,2.1vh,34px)]"
              >
                <div className="grid h-16 w-16 place-items-center rounded-full border-2 border-[#d8a14c] bg-white text-[#c9903f] shadow-[0_7px_18px_rgba(84,54,16,0.13)] sm:h-[clamp(72px,7.3vw,124px)] sm:w-[clamp(72px,7.3vw,124px)]">
                  <Icon className="h-9 w-9 sm:h-12 sm:w-12 lg:h-14 lg:w-14" strokeWidth={1.45} />
                </div>

                <h3 className="mt-4 font-serif text-[clamp(20px,4.5vw,42px)] font-light leading-tight text-[#214439] sm:mt-[clamp(14px,1.9vh,30px)]">
                  {card.title}
                </h3>

                <div className="my-4 flex w-[clamp(72px,20vw,138px)] items-center justify-center gap-3 text-[#c9903f] sm:my-[clamp(12px,1.6vh,24px)]">
                  <span className="h-0.5 flex-1 bg-[#d7b56c]" />
                  <span className="h-3 w-3 rotate-45 border border-[#c9903f]" />
                  <span className="h-0.5 flex-1 bg-[#d7b56c]" />
                </div>

                <p className="flex-1 text-[clamp(15px,3.8vw,27px)] font-light leading-[1.55] text-[#40515f]">
                  {card.text}
                </p>
