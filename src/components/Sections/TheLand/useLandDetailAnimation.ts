...
/**
 * Entrance animation for Land detail pages.
 * Hero, intro, cards, and panels animate in parallel on load.
 */
export function useLandDetailAnimation(deps: unknown[] = []) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const back = ".land-detail-back";
      const hero = ".land-detail-hero";
      const intro = ".land-detail-intro > *";
      const cards = ".land-detail-card";
      const panels = ".land-detail-panel";
      const extras = ".land-detail-extra";
