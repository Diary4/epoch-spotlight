...
export default function StoryOfResilience({ onBack }: StoryOfResilienceProps) {
  return (
    <main className="m-0 min-h-[100vh] w-[100vw] max-w-none bg-[#fbf3e8] px-[clamp(12px,1.8vw,28px)] py-[clamp(10px,1.6vh,24px)] text-[#174b3d]">
      <section className="relative mx-auto flex min-h-[calc(100vh-2*clamp(10px,1.6vh,24px))] w-[min(100vw,1400px)] max-w-none flex-col overflow-hidden rounded-[clamp(22px,2.4vw,34px)] bg-[#fff7ec] px-[clamp(20px,3.1vw,52px)] py-[clamp(20px,2.7vh,40px)]">
        <button
          type="button"
          onClick={onBack}
          className="absolute left-[clamp(16px,2vw,30px)] top-[clamp(16px,2vh,30px)] z-30 grid h-[clamp(50px,4.8vw,64px)] w-[clamp(50px,4.8vw,64px)] place-items-center rounded-full border-2 border-[#d9b477] bg-white/70 text-[#174b3d] shadow-sm"
        ...
        <div className="pointer-events-none absolute right-0 top-[clamp(130px,11vh,210px)] h-[clamp(780px,67vh,1160px)] w-[clamp(520px,52vw,900px)]">
          <img
        ...
        <section className="relative z-10 mt-[clamp(62px,8.2vh,126px)] max-w-[min(58vw,760px)]">
          <h1 className="font-serif text-[clamp(64px,8vw,116px)] font-semibold leading-[1.03] tracking-tight text-[#214439]">
            A Story of<br />Resilience
          </h1>

          <div className="mt-[clamp(26px,3.8vh,48px)] flex items-center gap-[clamp(14px,1.5vw,22px)] text-[#c9903f]">
          ...
          <p className="mt-[clamp(22px,3vh,40px)] font-serif text-[clamp(30px,3.6vw,52px)] leading-tight text-[#b06f25]">
            A history shaped by endurance,<br />dignity, and hope.
          </p>

          <p className="mt-[clamp(18px,2.8vh,38px)] max-w-[min(48vw,650px)] text-[clamp(18px,2vw,30px)] font-semibold leading-[1.75] text-[#35435b]">
        ...
        <div className="relative z-10 mt-[clamp(18px,2.5vh,36px)] h-[clamp(130px,15vh,250px)] max-w-[min(62vw,860px)] opacity-90">
          <img
        ...
        <section className="relative z-20 mt-[clamp(20px,4.2vh,70px)] grid grid-cols-3 gap-[clamp(16px,1.8vw,34px)] pb-[clamp(8px,1vh,22px)] pt-[clamp(20px,2.8vh,42px)]">
          {cards.map((card) => {
            ...
            return (
              <article
                key={card.title}
                className="relative flex min-h-[clamp(330px,30vh,500px)] flex-col items-center overflow-hidden rounded-[clamp(20px,2.2vw,32px)] border-2 border-[#f4e5cc] bg-white/82 px-[clamp(16px,1.8vw,34px)] py-[clamp(18px,2.1vh,34px)] text-center shadow-[0_14px_35px_rgba(84,54,16,0.16)] backdrop-blur-md"
              >
