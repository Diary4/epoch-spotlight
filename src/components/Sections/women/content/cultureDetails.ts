export const cultureDetailsByLang = {
  en: {
    "eyse-san": {
      nameLine1: "Eyşe",
      nameLine2: "Şan",
      role: "Singer · Voice of Kurdish Resistance · Cultural Icon",
      metaLine: "1938 – December 18, 1996 | Diyarbakir, Turkey",
      intro:
        "Eyşe Şan was born into a musical family, but her family told her: singing is a sin for women. She left them. She left her husband. She moved to a new city alone and began to sing. Her voice carried the pain and resistance of Kurdish people across Turkey, at a time when even speaking Kurdish in public was illegal. After the 1971 military coup banned the Kurdish language entirely, she emigrated to Germany. Her daughter died there. She disappeared from public life for years. Then she came back, and sang again.",
      portraitAlt: "Eyşe Şan",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "Greatest Achievement",
        text: "She preserved the Kurdish language through song during one of its most dangerous periods in Turkey. When the language itself was outlawed, her music kept it alive in the hearts of an entire generation in exile.",
      },
      whySheMatters: {
        title: "Why She Still Matters",
        text: "When a government bans a language, it tries to erase a people. Eyşe Şan sang that language in concert halls, in exile, through grief and loss. Language cannot be erased when someone keeps singing it.",
      },
      cards: [
        {
          icon: "♜",
          title: "Timeline Position",
          text: "Mid 20th Century, 1960s to 1990s",
        },
        {
          icon: "⛩",
          title: "Map Location",
          text: "Diyarbakir (Amed), Istanbul, Germany, and South Kurdistan",
        },
      ],
      didYouKnow: {
        title: "Did You Know?",
        text: "Eyşe Şan performed in South Kurdistan (Erbil region) in 1979, years before it was common or safe to do so. She crossed borders, physical and political, with her voice.",
      },
      quote: "Her voice was banned. She sang anyway.",
      quoteAuthor: "Kurdish cultural history",
    },
    "pakize-rafik-hilmi": {
      nameLine1: "Pakize",
      nameLine2: "Rafik Hilmi",
      role: "Harvard Linguist · First Kurdish Woman in the Kurdish Academy of Sciences",
      metaLine: "1924 – August 13, 2003 | Sulaymaniyah, Iraqi Kurdistan",
      intro:
        "In 1949, Pakize Rafik Hilmi was accepted by Harvard University's Department of Oriental Languages, becoming the first Iraqi student, male or female, to earn a master's degree there. She returned to Baghdad and became the first woman to head the Department of Kurdish Language at a major university. She taught at Berlin, Amman, and Baghdad. She wrote dictionaries. She gave lectures on Kurdish linguistics on Baghdad Radio. She worked until the last days of her life in 2003.",
      portraitAlt: "Pakize Rafik Hilmi",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "Greatest Achievement",
        text: "First Iraqi student to earn a Harvard master's degree (1951). First woman to head the Kurdish Language Department at Baghdad University (1958). First Kurdish woman elected to the Kurdish Academy of Sciences (1970). First Kurdish woman in the British Royal Asiatic Society.",
      },
      whySheMatters: {
        title: "Why She Still Matters",
        text: "Pakize Rafik Hilmi proved that Kurdish scholarship belonged on the world stage, at Harvard, in Berlin, in Amman, in the pages of academic journals. She opened a door that had never been opened before.",
      },
      cards: [
        {
          icon: "♜",
          title: "Timeline Position",
          text: "Mid to Late 20th Century, 1940s to 2003",
        },
        {
          icon: "⛩",
          title: "Map Location",
          text: "Sulaymaniyah, Baghdad, Harvard (USA), Berlin (Germany), Amman (Jordan)",
        },
      ],
      didYouKnow: {
        title: "Did You Know?",
        text: "Pakize's father was also a prominent Kurdish intellectual, and after his death she republished all his works, ensuring two generations of Kurdish scholarship survived.",
      },
      quote:
        "Until the day of her passing, she never ceased reading, writing, or engaging in intellectual and social activities.",
      quoteAuthor: "Kurdish Women's Library, Archive and Research Center",
    },
    "roshan-bedirkhan": {
      nameLine1: "Roshan",
      nameLine2: "Bedirkhan",
      role: "Writer · Cultural Pioneer · Activist",
      metaLine: "1909 – 1992",
      intro:
        "She was a prominent Kurdish intellectual and writer whose early life was defined by forced displacement from Kayseri to Istanbul and Damascus. As the wife and partner of Prince Jaladet Bedirkhan, she dedicated her life to the preservation of the Kurdish language and culture. She was a tireless activist who championed girls' education and worked to strengthen national identity through her prolific writing. Her upbringing, shaped by constant migration, served as the foundation for her lifelong commitment to fostering Kurdish awareness and advancing the national struggle.",
      portraitAlt: "Roshan Bedirkhan",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "Greatest Achievement",
        text: "She played a pivotal role as a partner in the publication of the journals Hawar and Ronahî, where she oversaw numerous editorial and administrative activities essential to preserving the Kurdish language. She was a prominent member of the \"Kurdish Women's Union\" beginning in 1934, consistently fighting for the rights of Kurdish women and the development of their educational and social capabilities. Additionally, she left a rich literary legacy through her memoirs and writings, which have been documented in numerous books and literary studies.",
      },
      whySheMatters: {
        title: "Why She Still Matters",
        text: "Roshan Bedirkhan remains an enduring symbol of the intellectual woman who did not just write, but took a leadership role in the cultural and political arena. Her legacy in preserving the Kurdish language and raising generations to value their national identity makes her one of the pillars of modern Kurdish thought, continuing to inspire activists and writers today.",
      },
      cards: [
        {
          icon: "♜",
          title: "Timeline Position",
          text: "July 11, 1909, until June 1, 1992.",
        },
        {
          icon: "⛩",
          title: "Map Location",
          text: "Born in Kayseri (Northern Kurdistan), she lived in Istanbul and Damascus; she passed away in Damascus and was laid to rest in the Sheikh Khalid Naqshbandi cemetery.",
        },
      ],
      didYouKnow: {
        title: "Did You Know?",
        text: "Although she was fluent in Turkish, Arabic, and French, she always insisted on speaking and writing in Kurdish, emphasizing that \"language is the essence of national identity, which must never be compromised under any circumstances\".",
      },
      quote:
        "The woman is the fundamental pillar of a nation's renaissance, and no people can be truly liberated unless their women are conscious and educated.",
      quoteAuthor: "Attributed to Roshan Bedirkhan",
    },
    "nahida-sheikh-salam": {
      nameLine1: "Nahida",
      nameLine2: "Sheikh Salam",
      role: "Writer · Cultural Pioneer · Activist",
      metaLine: "1922 – September 16, 1999 | Sulaymaniyah, Kurdistan Region of Iraq",
      intro:
        "She was a prominent Kurdish educator, poet, and writer from Sulaymaniyah who dedicated her life to nationalist activism and women's rights. Joining the \"Komeley Hiwa\" society in 1935, she balanced a long teaching career with steadfast support for the Kurdish cause. She was deeply respected for her collaboration with the Barzani movement and her role in mobilizing youth for political work. Furthermore, her home served as a vital sanctuary for those committed to Kurdish culture and the liberation struggle.",
      portraitAlt: "Nahida Sheikh Salam",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "Greatest Achievement",
        text: "Her primary legacy lies in pioneering girls' education and nationalist activism. After completing her studies at the Teachers' House in Baghdad in 1940, she established a school for girls in Sulaymaniyah, significantly advancing women's education in the city. She was also a prominent figure in the Kurdish women's movement, celebrated for her powerful poetry and dedication to preserving Kurdish language and culture.",
      },
      whySheMatters: {
        title: "Why She Still Matters",
        text: "Nahida Sheikh Salam remains an essential figure in Kurdish intellectual history. She proved that Kurdish women could simultaneously be pillars of the education system and frontline national activists. Her life serves as an enduring bridge between the early 20th-century movements for Kurdish consciousness and the modern era.",
      },
      cards: [
        {
          icon: "♜",
          title: "Timeline Position",
          text: "1922 until September 16, 1999.",
        },
        {
          icon: "⛩",
          title: "Map Location",
          text: "Sulaymaniyah, Kurdistan Region of Iraq.",
        },
      ],
      didYouKnow: {
        title: "Did You Know?",
        text: "She was deeply committed to the belief that the national struggle was not exclusively the domain of men and that women were equally capable of bearing the burden of national responsibility.",
      },
      quote:
        "I was the first Kurdish woman to join the struggle... because I believed that a woman should be a partner to a man in building the nation.",
      quoteAuthor: "Attributed to Nahida Sheikh Salam",
    },
  },
  ku: {
    "eyse-san": {
      nameLine1: "ئایشە",
      nameLine2: "شان",
      role: "گۆرانیبێژ · دەنگی بەرخۆدانی کورد · هێمای کولتووری",
      metaLine: "١٩٣٨ – ١٨ی کانوونی یەکەمی ١٩٩٦ | ئامەد، تورکیا",
      intro:
        "ئایشە شان لە خانوازێکی میوزیکی لەدایکبوو، بەڵام خێزانەکەی پێیان وت: گۆرانی خوێندن تاوانە بۆ ژنان. جیانیان بەجێهێشت. هاوسەرەکەی جیابووەوە. بە تەنیا گواسترایەوە بۆ شارێکی نوێ و دەستی بە گۆرانی خوێندن کرد. دەنگی ئازاری و بەرخۆدانی گەلی کورد لە تورکیا گەیاند, لە کاتێکدا تەنانەت قسەکردن بە کوردی لە گشتی نایاسایی بوو. دوای کۆدەتا سەربازی ١٩٧١ کە زمانی کوردی بە تەواوی قەدەغەکرا، کۆچی کرد بۆ ئەڵمانیا. کچەکەی لەوێ مرد. بۆ ساڵان لە ژیانی گشتی ونبوو. پاشان گەڕایەوە و دووبارە گۆرانی خوێند.",
      portraitAlt: "ئایشە شان",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "گەورەترین دەستکەوت",
        text: "زمانی کوردی لە ڕێگەی گۆرانییەوە لە یەکێک لە مەترسیدارترین سەردەمەکانی تورکیا پاراست. کاتێک خۆی زمانی قەدەغە کرابوو، میوزیکەکەی لە دڵی تەواوی نەوەیەک لە تاراوگەدا زیندوویی پێ بەخش.",
      },
      whySheMatters: {
        title: "بۆچی هێشتا گرنگە",
        text: "کاتێک حکومەت زمانێک قەدەغە دەکات، هەوڵ دەدات خەڵکێک بسڕێتەوە. ئایشە شان ئەو زمانە لە هۆڵی کۆنسێرت، لە تاراوگە، لە نێوان خەم و لەدەستداندا گۆڕی. زمان ناتوانرێت بسڕدرێتەوە کاتێک کەسێک بەردەوام دەبێت لە گۆرانی خوێندن.",
      },
      cards: [
        { icon: "♜", title: "شوێنی کات", text: "نیوەی سەدەی بیستەم, ١٩٦٠ەکان تا ١٩٩٠ەکان" },
        { icon: "⛩", title: "شوێنی نەخشە", text: "ئامەد، ئێستەنبوڵ، ئەڵمانیا، و باشووری کوردستان" },
      ],
      didYouKnow: {
        title: "ئایا دەزانی؟",
        text: "ئایشە شان لە باشووری کوردستان (ناوچەی هەولێر) لە ١٩٧٩ گۆرانی خوێند، ساڵان پێش ئەوەی ئەمە باو یان سەلامەت بێت. بە دەنگی خۆی سنوورەکان, فیزیکی و سیاسی, پەڕاند.",
      },
      quote: "دەنگی قەدەغە کرابوو. هەرچۆنێک بێت گۆرانی خوێند.",
      quoteAuthor: "مێژووی کولتووری کورد",
    },
    "pakize-rafik-hilmi": {
      nameLine1: "پاکیزە",
      nameLine2: "ڕەفیق حیلمی",
      role: "زمانناسی هارڤارد · یەکەم ژنانی کورد لە ئەکادیمیای زانستی کورد",
      metaLine: "١٩٢٤ – ١٣ی ئابی ٢٠٠٣ | سلێمانی، کوردستانی عێراق",
      intro:
        "لە ١٩٤٩، پاکیزە ڕەفیق حیلمی لە بەشی زمانە ڕۆژهەڵاتەکانی زانکۆی هارڤارد وەرگیرا, بوو بە یەکەم خوێندکاری عێراقی، نێر یان مێ، کە ماستەر لەوێ بەدەستهێنا. گەڕایەوە بۆ بەغدا و بوو بە یەکەم ژن کە سەرۆکایەتی بەشی زمانی کوردی لە زانکۆیەکی گەورە بکات. لە بەرلین، عەممان، و بەغدا وانە دەوت. فەرهەنگی نووسی. لە ڕادیۆی بەغدا وانەی زمانەوانی کوردی دا. تا ڕۆژی کۆتایی ژیانی لە ٢٠٠٣ کاریکرد.",
      portraitAlt: "پاکیزە ڕەفیق حیلمی",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "گەورەترین دەستکەوت",
        text: "یەکەم خوێندکاری عێراقی بوو کە ماستەری هارڤارد بەدەستهێنا (١٩٥١). یەکەم ژن بوو کە سەرۆکایەتی بەشی زمانی کوردی لە زانکۆی بەغدا بکات (١٩٥٨). یەکەم ژنانی کورد بوو کە هەڵبژێردرا بۆ ئەکادیمیای زانستی کورد (١٩٧٠). یەکەم ژنانی کورد لە کۆمەڵەی ڕۆیال ئاسیاتیکی بەریتانیا.",
      },
      whySheMatters: {
        title: "بۆچی هێشتا گرنگە",
        text: "پاکیزە ڕەفیق حیلمی سەلماند کە زانستی کوردی شایانی مێزەی جیهانە, لە هارڤارد، بەرلین، عەممان، لە لاپەڕەکانی گۆڤارە ئەکادیمییەکان. دەرگایەکی کردەوە کە پێشتر هەرگیز نەکرابووەوە.",
      },
      cards: [
        { icon: "♜", title: "شوێنی کات", text: "نیوەی دووەمی سەدەی بیستەم, ١٩٤٠ەکان تا ٢٠٠٣" },
        {
          icon: "⛩",
          title: "شوێنی نەخشە",
          text: "سلێمانی، بەغدا، هارڤارد (ئەمریکا)، بەرلین (ئەڵمانیا)، عەممان (ئوردن)",
        },
      ],
      didYouKnow: {
        title: "ئایا دەزانی؟",
        text: "باوکی پاکیزەش ڕۆشنبیرێکی بەناوبانگی کورد بوو، و دوای مردنی دووبارە هەموو بەرهەمەکانی بڵاوکردەوە, دڵنیابوو لە مانەوەی دوو نەوەی زانستی کوردی.",
      },
      quote:
        "تا ڕۆژی کۆتایی ژیانی، هەرگیز خوێندنەوە، نووسین، یان چالاکییە فیکری و کۆمەڵایەتییەکانی نەگرت.",
      quoteAuthor: "کتێبخانەی ژنانی کورد، ئەرشیف و ناوەندی لێکۆڵینەوە",
    },
    "roshan-bedirkhan": {
      nameLine1: "ڕەوشەن",
      nameLine2: "بەدرخان",
      role: "نووسەر · پێشەنگی کولتووری · چالاکوان",
      metaLine: "١٩٠٩ – ١٩٩٢",
      intro:
        "ڕۆشنبیر و نووسەرێکی بەناوبانگی کورد بوو کە ژیانی سەرەتایی بە گواستنەوەی ناچاری لە قەیسەری بۆ ئێستەنبوڵ و دیمەشق دیاری کرا. وەک هاوسەر و هاوبەشی شازادە جەلادەت بەدرخان، ژیانی تەرخانکرد بۆ پاراستنی زمانی و کولتووری کوردی. چالاکوانیەکی بێوەستان بوو کە بەرگری لە پەروەردەی کچان کرد و لە ڕێگەی نووسینە فراوانەکانیەوە کاریکرد بۆ بەهێزکردنی ناسنامەی نەتەوەیی.",
      portraitAlt: "ڕەوشەن بەدرخان",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "گەورەترین دەستکەوت",
        text: "ڕۆڵێکی گرنگی وەک هاوبەش لە بڵاوکردنەوەی گۆڤارەکانی هەوار و ڕۆناهی هەبوو، کە چەندین چالاکی دەستکاری و کارگێڕی گرنگ بۆ پاراستنی زمانی کوردی سەرپەرشتی دەکرد. ئەندامێکی بەناوبانگ بوو لە \"یەکێتی ژنانی کورد\" لە ١٩٣٤ەوە، بەردەوام شەڕی کرد بۆ مافەکانی ژنانی کورد و پەرەپێدانی توانا پەروەردەیی و کۆمەڵایەتییان.",
      },
      whySheMatters: {
        title: "بۆچی هێشتا گرنگە",
        text: "ڕەوشەن بەدرخان هێمایەکی بەردەوامە بۆ ژنی ڕۆشنبیر کە تەنها نەنووسی، بەڵکو ڕۆڵی سەرکردایەتی لە بواری کولتووری و سیاسی گرت. میراتەکەی لە پاراستنی زمانی کوردی و بەرزکردنەوەی نەوەکان بۆ بەنرخ دان بە ناسنامەی نەتەوەیی، یەکێک لە ستونەکانی بیرۆکەی مۆدێرنی کوردە.",
      },
      cards: [
        { icon: "♜", title: "شوێنی کات", text: "١١ی تەمموزی ١٩٠٩، تا ١ی حوزەیرانی ١٩٩٢." },
        {
          icon: "⛩",
          title: "شوێنی نەخشە",
          text: "لە قەیسەری (باکووری کوردستان) لەدایکبووە، لە ئێستەنبوڵ و دیمەشق ژیای کرد؛ لە دیمەشق کۆچی دوایی کرد و لە گۆڕستانی شێخ خالید نەقشبەندی نێژراوە.",
        },
      ],
      didYouKnow: {
        title: "ئایا دەزانی؟",
        text: "هەرچەندە بە تەواوی بە تورکی، عەرەبی، و فەرەنسی قسە دەکرد، هەمیشە پێداگری دەکرد لەسەر قسە و نووسین بە کوردی، جەخت لەسەر ئەوە دەکردەوە کە \"زمان بنەڕەتی ناسنامەی نەتەوەییە و نابێت لە هیچ بارودۆخێکدا لاسایی بکرێت\".",
      },
      quote:
        "ژن ستونی بنەڕەتی ڕەنەیسانسی نەتەوەیە، و هیچ گەلێک بە ڕاستی ناچارە ئازاد بێت مەگەر ژنەکانی ئاگادار و پەروەردەکراو بن.",
      quoteAuthor: "بە ڕەوشەن بەدرخان دەگەڕێتەوە",
    },
    "nahida-sheikh-salam": {
      nameLine1: "ناهیدە",
      nameLine2: "شێخ سەڵام",
      role: "نووسەر · پێشەنگی کولتووری · چالاکوان",
      metaLine: "١٩٢٢ – ١٦ی ئەیلوولی ١٩٩٩ | سلێمانی، هەرێمی کوردستانی عێراق",
      intro:
        "پەروەردەکار، شاعیر، و نووسەرێکی بەناوبانگی کورد لە سلێمانی بوو کە ژیانی تەرخانکرد بۆ چالاکوانی نەتەوەیی و مافی ژنان. لە ١٩٣٥ پەیوەندی بە کۆمەڵەی هیوا کرد، کارەری درێژی وانەوتنەوەی لەگەڵ پشتیوانی جێگیر لە کاری کورد تێکەڵ کرد. بە هاوکاری لەگەڵ بزووتنەوەی بارزانی و ڕۆڵی لە بەکارخستنی گەنجان بۆ کاری سیاسی ڕێزێکی قووڵی وەرگرت.",
      portraitAlt: "ناهیدە شێخ سەڵام",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "گەورەترین دەستکەوت",
        text: "میراتی سەرەکی لە پێشەنگی پەروەردەی کچان و چالاکوانی نەتەوەییدا دەبێت. دوای تەواوکردنی خوێندن لە خانەی مامۆستایان لە بەغدا لە ١٩٤٠، قوتابخانەیەکی کچان لە سلێمانی دامەزراند و پەروەردەی ژنان لە شارەکە بە شێوەیەکی بەرچاو پێشخست.",
      },
      whySheMatters: {
        title: "بۆچی هێشتا گرنگە",
        text: "ناهیدە شێخ سەڵام هێمایەکی گرنگە لە مێژووی ڕۆشنبیری کورد. سەلماند کە ژنانی کورد دەتوانن هەم ستونی سیستەمی پەروەردە بن و هەم چالاکوانی پێشەنگی نەتەوەیی. ژیانی پردێکە لە نێوان بزووتنەوەکانی سەرەتای سەدەی بیستەم بۆ هۆشیاری کورد و سەردەمی مۆدێرن.",
      },
      cards: [
        { icon: "♜", title: "شوێنی کات", text: "١٩٢٢ تا ١٦ی ئەیلوولی ١٩٩٩." },
        { icon: "⛩", title: "شوێنی نەخشە", text: "سلێمانی، هەرێمی کوردستانی عێراق." },
      ],
      didYouKnow: {
        title: "ئایا دەزانی؟",
        text: "بە توندی باوەڕی وابوو کە تێکۆشانی نەتەوەیی تایبەت بە پیاوان نەبوو و ژنان بە هەمان شێوە توانای هەڵگرتنی باری بەرپرسیارێتی نەتەوەییان هەبوو.",
      },
      quote:
        "من یەکەم ژنانی کورد بووم کە بەشداری تێکۆشانم کرد... چونکە باوەڕم وابوو ژن دەبێت هاوبەشی پیاو بێت لە دروستکردنی نەتەوە.",
      quoteAuthor: "بە ناهیدە شێخ سەڵام دەگەڕێتەوە",
    },
  },
  ar: {
    "eyse-san": {
      nameLine1: "عائشة",
      nameLine2: "شان",
      role: "مغنية · صوت المقاومة الكردية · أيقونة ثقافية",
      metaLine: "١٩٣٨ – ١٨ ديسمبر ١٩٩٦ | ديار بكر، تركيا",
      intro:
        "وُلدت عائشة شان في عائلة موسيقية، لكن عائلتها قالت لها: الغناء حرام للنساء. تركتهم. تركت زوجها. انتقلت إلى مدينة جديدة وحدها وبدأت تغني. حملت صوت ألم ومقاومة الشعب الكردي في تركيا, في وقت كان التحدث بالكردية في العلن محظوراً. بعد انقلاب 1971 العسكري الذي حظر اللغة الكردية بالكامل، هاجرت إلى ألمانيا. توفيت ابنتها هناك. اختفت من الحياة العامة لسنوات. ثم عادت، وغنت من جديد.",
      portraitAlt: "عائشة شان",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "أعظم إنجاز",
        text: "حفظت اللغة الكردية من خلال الأغنية خلال أحد أخطر فتراتها في تركيا. عندما حُظرت اللغة نفسها، أبقت موسيقاها اللغة حية في قلوب جيل بأكمله في المنفى.",
      },
      whySheMatters: {
        title: "لماذا لا تزال مهمة",
        text: "عندما تحظر حكومة لغة، تحاول محو شعب. غنت عائشة شان تلك اللغة في قاعات الحفلات، في المنفى، عبر الحزن والفقد. لا يمكن محو اللغة عندما يواصل أحد الغناء بها.",
      },
      cards: [
        { icon: "♜", title: "الموقع الزمني", text: "منتصف القرن العشرين, الستينيات إلى التسعينيات" },
        { icon: "⛩", title: "الموقع الجغرافي", text: "ديار بكر (أمد)، إسطنبول، ألمانيا، وجنوب كردستان" },
      ],
      didYouKnow: {
        title: "هل تعلم؟",
        text: "أدت عائشة شان في جنوب كردستان (منطقة أربيل) عام 1979، قبل سنوات من أن يصبح ذلك شائعاً أو آمناً. عبرت الحدود, المادية والسياسية, بصوتها.",
      },
      quote: "حُظر صوتها. غنت على أي حال.",
      quoteAuthor: "التاريخ الثقافي الكردي",
    },
    "pakize-rafik-hilmi": {
      nameLine1: "باكيزة",
      nameLine2: "رفيق حلمي",
      role: "لغوية هارفارد · أول امرأة كردية في الأكاديمية الكردية للعلوم",
      metaLine: "١٩٢٤ – ١٣ أغسطس ٢٠٠٣ | السليمانية، كردستان العراق",
      intro:
        "في 1949، قُبلت باكيزة رفيق حلمي في قسم اللغات الشرقية بجامعة هارفارد, لتصبح أول طالبة عراقية، ذكراً أو أنثى، تحصل على درجة الماجستير هناك. عادت إلى بغداد وأصبحت أول امرأة ترأس قسم اللغة الكردية في جامعة كبرى. درّست في برلين وعمّان وبغداد. كتبت معاجم. ألقت محاضرات عن اللغويات الكردية في إذاعة بغداد. عملت حتى آخر أيام حياتها في 2003.",
      portraitAlt: "باكيزة رفيق حلمي",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "أعظم إنجاز",
        text: "أول طالبة عراقية تحصل على ماجستير هارفارد (1951). أول امرأة ترأس قسم اللغة الكردية في جامعة بغداد (1958). أول امرأة كردية تُنتخب في الأكاديمية الكردية للعلوم (1970). أول امرأة كردية في الجمعية الآسيوية الملكية البريطانية.",
      },
      whySheMatters: {
        title: "لماذا لا تزال مهمة",
        text: "أثبتت باكيزة رفيق حلمي أن البحث الكردي ينتمي إلى الساحة العالمية, في هارفارد، وبرلين، وعمّان، وفي صفحات المجلات الأكاديمية. فتحت باباً لم يُفتح من قبل.",
      },
      cards: [
        { icon: "♜", title: "الموقع الزمني", text: "منتصف إلى أواخر القرن العشرين, الأربعينيات إلى 2003" },
        {
          icon: "⛩",
          title: "الموقع الجغرافي",
          text: "السليمانية، بغداد، هارفارد (الولايات المتحدة)، برلين (ألمانيا)، عمّان (الأردن)",
        },
      ],
      didYouKnow: {
        title: "هل تعلم؟",
        text: "كان والد باكيزة أيضاً مثقفاً كردياً بارزاً، وبعد وفاته أعادت نشر جميع أعماله, ضامنة بقاء جيلين من البحث الكردي.",
      },
      quote:
        "حتى يوم رحيلها، لم تتوقف عن القراءة والكتابة أو المشاركة في الأنشطة الفكرية والاجتماعية.",
      quoteAuthor: "مكتبة المرأة الكردية، الأرشيف ومركز البحوث",
    },
    "roshan-bedirkhan": {
      nameLine1: "روشن",
      nameLine2: "بديرخان",
      role: "كاتبة · رائدة ثقافية · ناشطة",
      metaLine: "١٩٠٩ – ١٩٩٢",
      intro:
        "كانت مثقفة وكاتبة كردية بارزة عُرفت حياتها المبكرة بالنزوح القسري من قيصري إلى إسطنبول ودمشق. بصفتها زوجة وشريكة للأمير جلادت بديرخان، كرّست حياتها للحفاظ على اللغة والثقافة الكردية. كانت ناشطة لا تكل تدافع عن تعليم الفتيات وتعمل على تعزيز الهوية الوطنية من خلال كتاباتها الغزيرة.",
      portraitAlt: "روشن بديرخان",
      listIcon: "crown" as const,
      greatestAchievement: {
        title: "أعظم إنجاز",
        text: "لعبت دوراً محورياً كشريكة في نشر مجلتي هاوار وروناهي، حيث أشرفت على العديد من الأنشطة التحريرية والإدارية الضرورية للحفاظ على اللغة الكردية. كانت عضواً بارزاً في \"اتحاد المرأة الكردية\" منذ 1934، تحارب باستمرار من أجل حقوق المرأة الكردية وتطوير قدراتها التعليمية والاجتماعية.",
      },
      whySheMatters: {
        title: "لماذا لا تزال مهمة",
        text: "تبقى روشن بديرخان رمزاً دائماً للمرأة المثقفة التي لم تكتفِ بالكتابة، بل تولت دوراً قيادياً في الساحة الثقافية والسياسية. يجعلها إرثها في الحفاظ على اللغة الكردية ورفع أجيال تقدّر هويتها الوطنية أحد أركان الفكر الكردي الحديث.",
      },
      cards: [
        { icon: "♜", title: "الموقع الزمني", text: "١١ يوليو 1909، حتى ١ يونيو 1992." },
        {
          icon: "⛩",
          title: "الموقع الجغرافي",
          text: "وُلدت في قيصري (شمال كردستان)، عاشت في إسطنبول ودمشق؛ توفيت في دمشق ودُفنت في مقبرة الشيخ خالد النقشبندي.",
        },
      ],
      didYouKnow: {
        title: "هل تعلم؟",
        text: "رغم إتقانها التركية والعربية والفرنسية، أصرت دائماً على التحدث والكتابة بالكردية، مؤكدة أن \"اللغة جوهر الهوية الوطنية التي لا يجوز التنازل عنها تحت أي ظرف\".",
      },
      quote:
        "المرأة هي الركيزة الأساسية لنهضة الأمة، ولا يمكن لأي شعب أن يتحرر حقاً ما لم تكن نساؤه واعيات ومتعلمات.",
      quoteAuthor: "منسوب إلى روشن بديرخان",
    },
    "nahida-sheikh-salam": {
      nameLine1: "ناهيدة",
      nameLine2: "شيخ سلام",
      role: "كاتبة · رائدة ثقافية · ناشطة",
      metaLine: "١٩٢٢ – ١٦ سبتمبر ١٩٩٩ | السليمانية، إقليم كردستان العراق",
      intro:
        "كانت مربية وشاعرة وكاتبة كردية بارزة من السليمانية كرّست حياتها للنشاط الوطني وحقوق المرأة. انضمت إلى جمعية \"كوميلي هيوا\" عام 1935، ووازنت بين مسيرة تدريس طويلة ودعم ثابت للقضية الكردية. حظيت باحترام عميق لتعاونها مع حركة البارزاني ودورها في حشد الشباب للعمل السياسي.",
      portraitAlt: "ناهيدة شيخ سلام",
      listIcon: "flower" as const,
      greatestAchievement: {
        title: "أعظم إنجاز",
        text: "يكمن إراثها الأساسي في ريادة تعليم الفتيات والنشاط الوطني. بعد إكمال دراستها في بيت المعلمين في بغداد عام 1940، أسست مدرسة للفتيات في السليمانية، مما قدّم تعليم المرأة في المدينة بشكل ملحوظ.",
      },
      whySheMatters: {
        title: "لماذا لا تزال مهمة",
        text: "تبقى ناهيدة شيخ سلام شخصية أساسية في التاريخ الفكري الكردي. أثبتت أن المرأة الكردية يمكن أن تكون في آن واحد ركيزة منظومة التعليم وناشطة وطنية في الصف الأول. حياتها جسر دائم بين حركات الوعي الكردي في أوائل القرن العشرين والعصر الحديث.",
      },
      cards: [
        { icon: "♜", title: "الموقع الزمني", text: "١٩٢٢ حتى ١٦ سبتمبر ١٩٩٩." },
        { icon: "⛩", title: "الموقع الجغرافي", text: "السليمانية، إقليم كردستان العراق." },
      ],
      didYouKnow: {
        title: "هل تعلم؟",
        text: "كانت ملتزمة بعمق بأن النضال الوطني ليس حكراً على الرجال وأن النساء قادرات بالقدر نفسه على تحمل عبء المسؤولية الوطنية.",
      },
      quote:
        "كنت أول امرأة كردية تنضم إلى النضال... لأنني آمنت أن المرأة يجب أن تكون شريكة للرجل في بناء الأمة.",
      quoteAuthor: "منسوب إلى ناهيدة شيخ سلام",
    },
  },
};
