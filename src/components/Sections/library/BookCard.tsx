...
export default function BookCard({ book, variant = "carousel", showRating = false }: BookCardProps) {
  ...
  if (variant === "shelf") {
    return (
      ...
      >
        <div
          className="relative h-44 w-28 overflow-hidden rounded-sm shadow-[4px_4px_12px_rgba(0,0,0,0.25)] sm:h-52 sm:w-32"
        ...
        <p className="mt-2 text-center font-serif text-xs text-[#0B1C14]">{book.title}</p>
        <p className="text-center text-[10px] text-[#8B7355]">{author?.name}</p>
      </Link>
    );
  }
  ...
  if (variant === "compact") {
    return (
      ...
      >
        ...
        <p className="mt-2 font-serif text-sm text-[#0B1C14] line-clamp-1">{book.title}</p>
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 font-serif text-sm font-medium text-[#0B1C14]">{book.title}</p>
      ...
      {showRating && book.rating && (
        <div className="mt-1 flex items-center gap-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <span
              key={i}
              className={`text-xs ${i < Math.floor(book.rating!) ? "text-[#C5A059]" : "text-[#D4C4A8]"}`}
            >
              ★
            </span>
          ))}
          <span className="ml-1 text-xs text-[#8B7355]">{book.rating}</span>
        </div>
      )}
    </Link>
  );
